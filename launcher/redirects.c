...
/*   Updated: 2025/04/01 19:14:21 by ssousmat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pipex_manage_input_heredoc(char *heredoc_lim, t_line *line)
{
	...
	ft_pipe_protected(heredocfd, line);
	while (1)
	{
		ft_printf_2("> ");
	...
	ft_dup2_protected(heredocfd[READ], STDIN_FILENO, line);
...
void	pipex_manage_input_redirect(t_command command, t_line *line)
{
	...
	ft_dup2_protected(aux_fd, STDIN_FILENO, line);
	close(aux_fd);
}
...
void	pipex_manage_output_redirect(t_command command, t_line *line)
{
	...
	ft_dup2_protected(aux_fd, STDOUT_FILENO, line);
	close(aux_fd);
}
...
void	son_redirects(t_line *line, size_t cmd_index)
{
	...
	if (line->commands[cmd_index].append_input)
		pipex_manage_input_heredoc(line->commands[cmd_index].append_input, line);
	else if (line->commands[cmd_index].redirect_input)
		pipex_manage_input_redirect(line->commands[cmd_index], line);
	else if (cmd_index > 0)
	{
		ft_dup2_protected(line->pipefd[READ], STDIN_FILENO, line);
	}
	close(line->pipefd[READ]);
	if (line->commands[cmd_index].redirect_output || line->commands[cmd_index].append_output)
		pipex_manage_output_redirect(line->commands[cmd_index], line);
	else if (cmd_index + 1 < line->ncommands)
	{
		ft_dup2_protected(line->pipefd[WRITE], STDOUT_FILENO, line);
	}
